...
// Exercise the real runtime, adapter, and session hook with synthetic status
// markers. This tests lifecycle/session identity, not bundled Jcode screen rules;
// real Jcode screen compatibility is covered by live smoke validation.
#[test]
fn jcode_agent_start_and_prompt_keep_pane_session_identity() {
    use std::os::unix::fs::PermissionsExt;
    ...
    // Override only this disposable server's manifest. Jcode is the registry key
    // needed by the adapter, while the rules belong entirely to this fixture.
    let manifests = config_home.join(app_dir_name()).join("agent-detection");
    fs::create_dir_all(&manifests).unwrap();
    fs::write(
        manifests.join("jcode.toml"),
        r#"id = "jcode"
...
line_regex = ['^HERDR_FIXTURE_WORKING$']

[[rules]]
id = "fixture_idle"
state = "idle"
visible_idle = true
line_regex = ['^HERDR_FIXTURE_IDLE$']
"#,
    )
    .unwrap();
    let hook = base.join("herdr-jcode-hook.sh");
    fs::write(&hook, JCODE_HOOK_ASSET).unwrap();
    let fake_jcode = bin.join("jcode");
    fs::write(
        &fake_jcode,
        format!(
            "#!/bin/sh\nexport JCODE_HOOK_SESSION_ID=\"$1\" JCODE_HOOK_SOURCE=create\nprintf '\\033[2J\\033[HHERDR_FIXTURE_IDLE\\n'\nbash '{}'\nwhile IFS= read -r prompt; do\n  printf '\\033[2J\\033[HHERDR_FIXTURE_WORKING\\n'\n  sleep 0.3\n  printf '%s\\n' \"$prompt\" >> '{}/prompt-'\"$1\"\n  printf '\\033[2J\\033[HHERDR_FIXTURE_IDLE\\n'\ndone\n",
            hook.display(),
            base.display(),
        ),
    )
    .unwrap();
    fs::set_permissions(&fake_jcode, fs::Permissions::from_mode(0o755)).unwrap();
    ...
    for name in ["jcode-one", "jcode-two"] {
        ...
        assert_eq!(
            started["result"]["argv"],
            serde_json::json!(["jcode", name])
        );
        assert_eq!(started["result"]["agent"]["agent"], "jcode");
        let agent = run_cli_json(&socket_path, &["agent", "get", name]);
        let session = &agent["result"]["agent"]["agent_session"];
        assert_eq!(session["source"], "herdr:jcode");
        assert_eq!(session["agent"], "jcode");
        assert_eq!(session["kind"], "id");
        assert_eq!(session["value"], name);
        panes.push((name, pane));
    }
    assert_ne!(panes[0].1, panes[1].1);

    for (name, pane) in &panes {
        let prompted = run_cli_json(
            &socket_path,
            &[
                "agent",
                "prompt",
        ...
        assert_eq!(prompted["result"]["agent"]["pane_id"], *pane);
        assert_eq!(prompted["result"]["agent"]["agent_session"]["value"], *name);
        assert_eq!(
            fs::read_to_string(base.join(format!("prompt-{name}"))).unwrap(),
            "hello\n"
        );
    }
...
impl LifecycleHarness {
    ...
    fn assert_state(&self, state: State, edge: &Edge, side: &str) {
        ...
        if state.has_managed_hook_file() {
            ...
            assert_eq!(
                mode & 0o7777,
                0o600,
                "{} {side}: hook must remain private and interpreter-only",
                edge.name
            );
        }
    }
    ...
    fn invoke_hook(
        ...
        // Match the installed command's interpreter invocation. Resolve bash
        // independently of the reduced PATH used by dependency-failure cases.
    ...
    fn run_outside_herdr(&self, edge: &Edge) {
        let socket = FakeHookSocket::start_with_timeout(1, Duration::from_millis(700));
