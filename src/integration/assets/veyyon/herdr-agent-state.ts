...
const idleDebounceMs = parseDurationEnv("HERDR_VEYYON_IDLE_DEBOUNCE_MS", 250);
const retryGraceMs = parseDurationEnv("HERDR_VEYYON_RETRY_GRACE_MS", 2500);
...
export default function (pi) {
  ...
  pi.on("agent_end", (event) => {
    ...
    if (!agentActive) {
      // Veyyon can emit duplicate/late end events while auto-retry is already
