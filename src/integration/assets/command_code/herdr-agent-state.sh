...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/herdr-command-code-hook.XXXXXX")" || exit 0
...
# Only report on SessionStart payloads
hook_event_name = hook_input.get("hook_event_name") or ""
if hook_event_name not in ("SessionStart", ""):
    raise SystemExit(0)
