...
use super::targets::{
    install_antigravity_cli, install_claude, install_codex, install_command_code, install_copilot,
    ...
    install_qwen, uninstall_antigravity_cli, uninstall_claude, uninstall_codex,
    uninstall_command_code, uninstall_copilot, uninstall_cursor, uninstall_devin, uninstall_droid,
...
fn install_target_inner(target: crate::api::schema::IntegrationTarget) -> io::Result<Vec<String>> {
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::CommandCode => {
            let installed = install_command_code()?;
            vec![
                format!(
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::CommandCode => {
            let result = uninstall_command_code()?;
            let mut messages = Vec::new();
            if result.removed_hook_file {
                messages.push(format!(
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed herdr command-code hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no herdr command-code hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
