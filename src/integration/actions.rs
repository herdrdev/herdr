...
use super::targets::{
    install_claude, install_codewhale, install_codex, install_copilot, install_cursor,
    ...
    uninstall_codewhale, uninstall_codex, uninstall_copilot, uninstall_cursor, uninstall_devin,
...
fn install_target_inner(target: crate::api::schema::IntegrationTarget) -> io::Result<Vec<String>> {
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Codewhale => {
            let installed = install_codewhale()?;
            vec![
                format!(
                    "installed codewhale integration hook to {}",
                    installed.hook_path.display()
                ),
                format!(
                    "ensured codewhale config at {}",
                    installed.config_path.display()
                ),
            ]
        }
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Codewhale => {
            let result = uninstall_codewhale()?;
            let mut messages = Vec::new();
            if result.removed_hook_file {
                messages.push(format!(
                    "removed codewhale hook at {}",
                    result.hook_path.display()
                ));
            } else {
                messages.push(format!(
                    "no codewhale hook found at {}",
                    result.hook_path.display()
                ));
            }
            if result.updated_config {
                messages.push(format!(
                    "removed herdr codewhale hook entries from {}",
                    result.config_path.display()
                ));
            } else {
                messages.push(format!(
                    "no herdr codewhale hook entries found in {}",
                    result.config_path.display()
                ));
            }
            messages
