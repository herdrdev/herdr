...
impl VirtualEnvActivation {
    ...
    pub(crate) fn path_entries(&self) -> Vec<std::path::PathBuf> {
        self.path_entries_on(cfg!(windows))
    }

    /// The layout the host uses is taken as an argument so both layouts stay
    /// covered by tests wherever they run.
    fn path_entries_on(&self, windows: bool) -> Vec<std::path::PathBuf> {
        let prefix = &self.prefix;
        match (self.kind, windows) {
    ...
    pub(crate) fn launch_env(
        ...
        let prefix = std::ffi::OsString::from(&self.prefix);
        let name = self.name.clone().map(std::ffi::OsString::from);
        let mut env = match self.kind {
            VirtualEnvKind::Conda => {
                let mut vars = vec![("CONDA_PREFIX", prefix)];
                if let Some(name) = name {
                    vars.push(("CONDA_DEFAULT_ENV", name));
                }
                vars.push(("CONDA_SHLVL", "1".into()));
                vars
            }
            VirtualEnvKind::Venv => {
                let mut vars = vec![("VIRTUAL_ENV", prefix)];
                if let Some(name) = name {
                    vars.push(("VIRTUAL_ENV_PROMPT", name));
                }
                vars
            }
        };

        env.push(("CONDA_AUTO_ACTIVATE", "false".into()));
        env.push(("CONDA_AUTO_ACTIVATE_BASE", "false".into()));

        let entries = self.path_entries();
        let inherited = base_path.unwrap_or_else(|| std::ffi::OsStr::new(""));
...
mod tests {
    ...
    fn path_value(env: &[(&str, std::ffi::OsString)]) -> Vec<std::path::PathBuf> {
        let path = env
            .iter()
            .find(|(key, _)| *key == "PATH")
    ...
    fn launch_env_of(
        ...
        activation.launch_env(base_path.map(std::ffi::OsStr::new))
    }
    ...
    fn launch_env_puts_the_environment_first_on_the_inherited_path() {
        ...
        let env = launch_env_of(&activation, Some("/usr/local/bin:/usr/bin"));

        assert!(env.contains(&("VIRTUAL_ENV", "/work/api/.venv".into())));
        assert_eq!(
            path_value(&env),
            [
    ...
    fn launch_env_does_not_repeat_entries_already_on_the_inherited_path() {
        ...
        let env = launch_env_of(&activation, Some("/opt/conda/envs/web/bin:/usr/bin"));
    ...
    fn launch_env_stops_conda_from_auto_activating_base_over_the_restored_env() {
        ...
        for kind in [VirtualEnvKind::Conda, VirtualEnvKind::Venv] {
            ...
            let env = launch_env_of(&activation, Some("/usr/bin"));

            assert!(env.contains(&("CONDA_AUTO_ACTIVATE", "false".into())));
            assert!(env.contains(&("CONDA_AUTO_ACTIVATE_BASE", "false".into())));
        }
    }
    ...
    fn launch_env_pins_conda_nesting_depth_to_a_single_activation() {
        ...
        let env = launch_env_of(&activation, None);
