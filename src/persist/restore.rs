...
fn restore_tab(
    ...
    for id in &pane_ids {
        ...
        let extra_env = restored_virtual_env
            .as_ref()
            .map(|activation| activation.launch_env(std::env::var("PATH").ok().as_deref()))
            .unwrap_or_default();
        let launch_env = public_pane_id
            .map(|pane_id| {
                PaneLaunchEnv::from_extra(extra_env.clone()).with_identity(
                    workspace_id.to_string(),
                    crate::workspace::public_tab_id_for_number(workspace_id, number),
                    pane_id.to_string(),
                )
            })
            .unwrap_or_else(|| PaneLaunchEnv::from_extra(extra_env));
