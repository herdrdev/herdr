...
#[test]
fn kiro_manifest_detects_prompt_idle_and_working_states() {
    // Plain idle: prompt footer visible at bottom
    let idle = explain(
        Agent::Kiro,
        "─────────────────────────────────────────────────────────\n\
kiro_default · auto · ◔ 2%                         D:\\tmp\n\n\
 ask a question or describe a task ↵\n\
    ...
    // Plain working: spinner + "Kiro is working" replaces prompt
    let working = explain(
        Agent::Kiro,
...
kiro_default · auto · ◔ 2%                         D:\\tmp\n\n\
 Kiro is working · Type to steer · Ctrl+S to queue",
    ...
    // Working overlap: idle anchors + working marker in same region — not gate
    // vetoes idle, working wins
    let working_overlap = explain(
        Agent::Kiro,
        " ask a question or describe a task ↵\n\
                                      /copy to clipboard\n\
 Kiro is working · Type to steer · Ctrl+S to queue",
    );
    assert_eq!(working_overlap.state, AgentState::Working);
    assert_eq!(
        working_overlap
            .matched_rule
            .as_ref()
    ...
    assert!(working_overlap.visible_working);
    assert!(!working_overlap.visible_idle);

    // Spinner overlap: idle anchors + "esc to cancel" in same region — not gate
    // vetoes idle, spinner working rule wins
    let spinner_overlap = explain(
        Agent::Kiro,
        " ask a question or describe a task ↵\n\
                                      /copy to clipboard\n\
 ◑ Reading file.rs (esc to cancel)",
    );
    assert_eq!(spinner_overlap.state, AgentState::Working);
    assert_eq!(
        spinner_overlap
            .matched_rule
            .as_ref()
            .map(|rule| rule.id.as_str()),
        Some("tool_spinner_working")
    );
    assert!(spinner_overlap.visible_working);
    assert!(!spinner_overlap.visible_idle);

    // Stale working above live idle footer: priority 200 idle overrides
    // stale whole_recent working evidence at priority 100
    let stale_working = explain(
        Agent::Kiro,
        "Kiro is working · Type to steer · Ctrl+S to queue\n\
...
kiro_default · auto · ◔ 2%                         D:\\tmp\n\n\
 ask a question or describe a task ↵\n\
    ...
    assert_eq!(stale_working.state, AgentState::Idle);
    assert_eq!(
        stale_working
            .matched_rule
            .as_ref()
    ...
    assert!(stale_working.visible_idle);

    // Blocked precedence: approval dialog + idle footer — blocked wins at
    // priority 300 over idle at 200
    let blocked = explain(
        Agent::Kiro,
        "requires approval\n\
 ◑ Reading D:\\code\\file.rs\n\
