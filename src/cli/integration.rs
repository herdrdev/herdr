...
fn integration_status(args: &[String]) -> std::io::Result<i32> {
    ...
    if let Some(status) = crate::integration::experimental_jcode_integration_status() {
        let state = describe_integration_state(
            status.state,
            status.installed_version,
            status.expected_version,
...
fn integration_install(args: &[String]) -> std::io::Result<i32> {
    ...
    let installed = match target {
        ...
        IntegrationCommandTarget::Jcode => crate::integration::install_experimental_jcode(),
    };
    match installed {
        Ok(messages) => {
...
fn integration_uninstall(args: &[String]) -> std::io::Result<i32> {
    ...
    let removed = match target {
        ...
        IntegrationCommandTarget::Jcode => crate::integration::uninstall_experimental_jcode(),
    };
    match removed {
        Ok(messages) => {
            print_integration_messages(messages);
...
/// Integration target accepted by the CLI. Letta and Jcode are deliberately kept
...
fn parse_integration_target(
    ...
    let Some(target) = args.first().map(|arg| arg.as_str()) else {
        eprintln!(
            "usage: herdr integration {action} <pi|omp|claude|codex|copilot|devin|droid|kimi|opencode|kilo|hermes|qodercli|qwen|letta|cursor|mastracode|antigravity-cli|grok|jcode>"
        );
        return Ok(None);
    };
    if args.len() != 1 {
    ...
    let parsed = match target {
        ...
        "jcode" => IntegrationCommandTarget::Jcode,
        _ => {
            eprintln!("unknown integration target: {target}");
            eprintln!(
                "currently supported: pi, omp, claude, codex, copilot, devin, droid, kimi, opencode, kilo, hermes, qodercli, qwen, letta, cursor, mastracode, antigravity-cli, grok, jcode"
            );
            return Ok(None);
        }
    };
