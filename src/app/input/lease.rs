...
impl InputLeaseTable {
    ...
    /// Whether this key is currently leased to a terminal, meaning repeats
    /// belong to that pane even if the mode has moved off the terminal since.
    pub(crate) fn is_forwarded(&self, key: &InputLeaseKey) -> bool {
        matches!(self.leases.get(key), Some(InputLease::Forwarded(_)))
    }
