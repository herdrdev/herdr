...
| Clipboard image paste to agents in local panes | supported |
...
The configured image paste key reads a Windows clipboard image and sends it to the focused local or remote pane. Dropping one local image file into Windows Terminal also sends that file to the focused pane.
...
Herdr reads clipboard images itself and pastes a staged image path into the focused agent pane. This keeps image paste consistent across local native panes and `herdr --remote` sessions. The focused agent still needs to support image paths in its input flow.
