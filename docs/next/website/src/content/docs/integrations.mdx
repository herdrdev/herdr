---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and Cline CLI.
---
...
herdr integration uninstall cline
```
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, Cline CLI | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Install the Cline CLI hooks:

```bash
herdr integration install cline
```

The hooks report Cline session identity and lifecycle state to Herdr for native restore and authoritative `idle` and `working` status. Visible approval prompts still come from Herdr's screen manifest detection.

Herdr uses `~/.cline` by default, or `CLINE_DIR` when set. The Cline config directory must already exist. Cline discovers hook files by name inside `hooks/`, so install writes a reporter script plus one small per-event wrapper file for each Cline lifecycle event (`taskstart.sh`, `taskresume.sh`, `userpromptsubmit.sh`, `pretooluse.sh`, `posttooluse.sh`, `taskcomplete.sh`, `taskcancel.sh`, and `taskerror.sh`). Uninstall removes only the Herdr-owned files and never touches other hooks in that directory.

The Cline integration is not currently supported on Windows because Cline dispatches hook files through `bash`/`node` runners that may not exist there. Screen detection still works without the hooks.

Herdr resumes stored Cline sessions with `cline --id <id>`.
