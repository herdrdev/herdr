---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and Muse.
---
...
herdr integration install muse
```
...
herdr integration uninstall muse
```
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, Muse | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Install the Muse hook:

```bash
herdr integration install muse
```

The hook reports Muse session identity and lifecycle state to Herdr for native restore and authoritative `idle`, `working`, and `blocked` status.

Herdr uses `$XDG_CONFIG_HOME/muse` when `XDG_CONFIG_HOME` is set. Otherwise, it uses `~/.config/muse` on Linux and macOS or `%APPDATA%\muse` on Windows. The Muse config directory must already exist. Install writes `hooks/herdr-agent-state.sh` (`hooks/herdr-agent-state.ps1` on Windows) and merges Herdr-managed hook entries into the user-owned `settings.json`, preserving unrelated hooks. Uninstall removes the Herdr-managed hook entries and deletes the hook script.

Herdr resumes stored Muse sessions with `muse resume <id>`.

| Muse hook event | Herdr effect |
| --- | --- |
| `SessionStart` | Reports the native session id for restore. Fires once per session. |
| `UserPromptSubmit` | Reports `working`. |
| `PreToolUse` | Reports `working`. |
| `PostToolUse` | Reports `working`. |
| `PermissionRequest` | Reports `blocked`. |
| `Stop` | Reports `idle`. |
| `Interrupt` | Reports `idle`. |
| `SessionEnd` | Ends the session report; lifecycle release follows process exit. |
