...
[Prime Agent's built-in Herdr reporter](https://github.com/PrimeIntellect-ai/prime-agent/blob/main/packages/coding-agent/src/core/extensions/builtin/herdr-agent-state.ts) is a real-world example.
...
Command Code reports to Herdr from a built-in mod, so there is nothing to install. Start `cmd` in a Herdr pane to begin:

The mod activates when `HERDR_ENV=1`, `HERDR_SOCKET_PATH`, and `HERDR_PANE_ID` are set. It reports `idle`, `working`, and `blocked` (permission prompts and questions) as source `custom:cmd`, reports the session id, shows the model and session title as pane metadata, and releases the pane on exit. This works in interactive sessions and headless mode through `cmd -p`. Nested `cmd` processes started from inside that session leave the pane alone.
