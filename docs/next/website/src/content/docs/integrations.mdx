...
herdr integration install command-code
```
...
herdr integration uninstall command-code
```
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI, Command Code | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Install the Command Code hook:

```bash
herdr integration install command-code
```

The hook reports session identity through Command Code's `SessionStart` hook while Command Code runs inside a Herdr pane. Command Code state comes from Herdr's screen manifest detection.

Herdr uses `~/.commandcode` by default, or `COMMANDCODE_CONFIG_DIR` when set. The Command Code config directory must already exist. Install writes `hooks/herdr-agent-state.sh` (`hooks/herdr-agent-state.ps1` on Windows) and adds a Herdr `SessionStart` entry to `settings.json`. Uninstall removes the matching hook entries and deletes the hook script.

After Command Code emits a session start event, Herdr can use the reported session id to resume the pane with `command-code --resume <id>`.
