...
Herdr uses `~/.gemini/antigravity-cli/` by default, or `ANTIGRAVITY_CLI_CONFIG_DIR` when set. The directory is created if it does not exist. Install writes `hooks/herdr-agent-state.sh` (or `herdr-agent-state.ps1` on Windows) and adds Herdr entries to `hooks.json`. Uninstall removes the matching hook entries and deletes the hook script.

After Antigravity CLI emits a session-bearing event, Herdr can use the reported conversation ID to resume the pane with `agy --conversation <id>` after a Herdr server restart.
