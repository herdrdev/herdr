---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Veyyon, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
herdr integration install veyyon
...
herdr integration uninstall veyyon
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Veyyon, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Install the Veyyon integration:

```bash
herdr integration install veyyon
```

Herdr writes the bundled extension to `~/.veyyon/profiles/default/agent/extensions/herdr-veyyon-agent-state.ts`. `VEYYON_CODING_AGENT_DIR` overrides the complete agent directory. Otherwise, `VEYYON_CONFIG_DIR` selects the config root and `VEYYON_PROFILE` selects the profile. Herdr creates the extensions directory when the resolved Veyyon agent directory already exists. Uninstall removes only that extension file.

The extension reuses Veyyon's inherited Pi/OMP lifecycle events to report `working`, `blocked`, `idle`, and native session identity through Herdr's socket API. Without the extension, Herdr falls back to Veyyon's screen manifest. Herdr resumes stored sessions with `veyyon --resume <session>`.
