---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and Jcode.
---
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI, Jcode | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Jcode integration installation is experimental and CLI-only. Use `herdr integration install jcode`, `herdr integration uninstall jcode`, and `herdr integration status` to manage it. It is not listed in the Settings integration screen or the JSON integration API. Status reports it as `jcode (experimental)`. Agent detection, agent start, and prompt support remain available independently through dynamic agent kinds.
...
The integration requires Jcode `0.68.0` or later, which supports lifecycle hook command arrays.

Herdr uses `JCODE_HOME` when set, otherwise `~/.jcode`. The Jcode config directory must already exist. On Unix, install writes `hooks/herdr-agent-state.sh`, which requires `bash` and `python3`. On Windows, install writes `hooks/herdr-agent-state.ps1`, which uses PowerShell and the native Herdr CLI.

Install appends the platform-specific hook command to `[hooks].session_start` in `config.toml`. Jcode runs each command in that lifecycle hook array directly. Reinstalling does not duplicate the Herdr command, and uninstall removes only the Herdr command while preserving every other observer.

After Jcode emits a session start event, Herdr can resume the pane with `jcode --resume <id>` after a server restart only if a Jcode transcript has been saved. Empty, newly cleared sessions are not durable until a prompt saves them. The integration reports session identity only and never reports lifecycle state.
