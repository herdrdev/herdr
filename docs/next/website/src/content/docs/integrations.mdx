...
Command Code reports to Herdr from a built-in mod, so there is nothing to install and it does not appear in `herdr integration install`. Start `cmd` in a Herdr pane:

```bash
cmd
```

The mod activates only when `HERDR_ENV=1`, `HERDR_SOCKET_PATH`, and `HERDR_PANE_ID` are set. It reports `idle`, `working`, and `blocked` (permission prompts and questions) as source `custom:cmd`, reports the session id, shows the model and session title as pane metadata, and releases the pane on exit. This works in interactive sessions and `cmd -p`. Nested `cmd` processes started from inside that session leave the pane alone.

Run `/herdr` inside Command Code to see what it is reporting. Set `CMD_HERDR=0` or add `herdr` to `mods.disabled` in Command Code settings to turn it off.

Herdr does not resume Command Code sessions after a server restart yet; run `cmd --resume <id>` yourself.
