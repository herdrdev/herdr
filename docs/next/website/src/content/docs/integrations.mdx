...
herdr integration uninstall codewhale
```
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, MastraCode, Codewhale | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Install the Codewhale hook:

```bash
herdr integration install codewhale
```

The hook reports Codewhale lifecycle state and session identity to Herdr for authoritative `idle`, `working`, and `blocked` status and native restore. Codewhale has no screen manifest fallback; state comes from the hook while Codewhale runs inside a Herdr pane.

Herdr uses `~/.codewhale` (or `CODEWHALE_CONFIG_DIR` when set). Install writes `hooks/herdr-agent-state.sh` and registers hook entries under `[hooks]` in `config.toml`, creating the directory and config when missing. Uninstall removes the managed Herdr hook block and deletes the hook script.

Herdr resumes stored Codewhale sessions with `codewhale --resume <id>`.
