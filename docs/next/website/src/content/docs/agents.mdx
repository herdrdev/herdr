...
**Built-in agent watcher and titles.** This distribution starts its agent watcher with the Herdr server. It keeps the watcher state socket compatible with the standalone `herdr-agent-watcher` plugin, so existing watcher clients continue to work. It also derives pane labels from Claude Code, Codex, Kimi Code, and OpenCode session data. A label you rename manually remains yours; automatic title sync only updates labels it owns.

Use the native watcher commands without opening a plugin pane:

```bash
herdr watcher status
herdr watcher doctor
herdr watcher sidebar
herdr watcher claude-bridge enable
herdr watcher claude-bridge disable
herdr watcher kimi-consent on
herdr watcher kimi-consent off
herdr watcher kimi-consent status
```

Both built-in services are enabled by default. Their configuration is startup-only, so restart the Herdr server after changing it. `title_sync.interval_ms` must be a positive integer.
...
**Migrating from standalone plugins.** Disable the standalone twins before relying on the built-ins:

```bash
herdr plugin disable herdr-agent-watcher
herdr plugin disable herdr-agent-title-sync
herdr watcher doctor
```

Disabling leaves the plugin checkouts and state in place. You can re-enable them when returning to stock Herdr. If an enabled standalone twin remains registered while its built-in twin is enabled, `herdr watcher doctor` and the server startup log name the exact disable command. Herdr does not disable plugins automatically.

## Agent cards in the sidebar

The expanded sidebar shows each detected agent as a three-line card. The card combines Herdr's pane lifecycle state with optional watcher telemetry such as context use, tool calls, model, cache, cost, and recent traces. The focused pane expands when space permits; click its chevron to collapse or reopen it. If watcher telemetry is unavailable, navigation and lifecycle state continue to work and the card says `no telemetry`.

Cards follow the existing Agents sort control. `spaces` keeps a flat workspace-order list, while `priority` puts agents needing attention first. Indexed and previous/next agent focus use the same visible order as the cards.

These settings reload live:

```toml
[ui.sidebar]
agents_view = "cards"      # default; use "legacy" for configurable rows
agents_hide_idle = false   # hide seen idle agents from cards and focus indexes
compact_rail_numbers = true # set false to show state dots without position numbers
```

Switching to `legacy` restores the configurable `[ui.sidebar.agents]` row view. Agent row settings are ignored in cards mode and produce a configuration diagnostic when explicitly present. Compact-rail numbering reloads live and does not change row selection or click targets.
