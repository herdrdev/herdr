...
On Windows, each build command and its descendants run in an install-owned job.
When the build command exits, Herdr terminates any remaining descendants before
moving or removing the checkout. Build commands must finish their work before
exiting; launch runtime processes from plugin actions or startup hooks instead.
