...
| 缩放聚焦的窗格 | `prefix+z` |
| 关闭窗格 | `prefix+x` |
| 交换窗格 | `prefix+shift+h/j/l`, `prefix+ctrl+k` |
| 调整大小模式 | `prefix+r` |
| 复制模式 | `prefix+[` |
| 摘取可见 token | `prefix+s` |
| 打开已安装的 Sesh 选择器 | `prefix+shift+k` |
...
按 `prefix+[` 让聚焦的窗格进入复制模式。用 `h/j/k/l`、tmux 风格的 `w/b/e` 和 `{`/`}` 移动。按 `/` 或 `?` 向前或向后进行文本搜索,再用 `n` 或 `N` 按相同或相反方向重复搜索。查询包含大写字母时区分大小写,否则不区分。用 `v` 或空格开始选择,用 `y` 或回车复制,用 `q` 或 Esc 不复制直接退出。Esc 会先清除当前选择或搜索,然后才退出。按 `prefix+s` 会给可见的 URL、路径、哈希、IP 地址和长数字加上内联提示;输入提示会复制对应 token,Esc 或 `ctrl+c` 会取消。复制模式不会暂停窗格进程:停留在底部时继续跟随输出,进入历史记录后保持当前位置。鼠标拖选可以直接复制,完全不用进入复制模式。
