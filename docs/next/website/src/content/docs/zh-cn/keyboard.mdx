...
按住鼠标左键拖动可选择普通终端文本。按住 `alt` 拖动可选择并复制矩形区域，即使窗格内的程序已启用鼠标报告也能使用。设置 `ui.copy_on_select = false` 后，两种选择都会保持高亮，直到按 `ctrl+c`，或在宿主终端会转发时按 `cmd+c`，复制并清除选择。

## 复制模式

按 `prefix+[` 让聚焦的窗格进入复制模式。用 `h/j/k/l`、tmux 风格的 `w/b/e` 和 `{`/`}` 移动。按 `/` 或 `?` 向前或向后进行文本搜索,再用 `n` 或 `N` 按相同或相反方向重复搜索。查询包含大写字母时区分大小写,否则不区分。用 `v` 或空格开始字符选择,用 `shift+v` 开始整行选择,用 `ctrl+v` 开始矩形选择。用 `y` 或回车复制,用 `q` 或 Esc 不复制直接退出。Esc 会先清除当前选择或搜索,然后才退出。复制模式不会暂停窗格进程:停留在底部时继续跟随输出,进入历史记录后保持当前位置。鼠标拖选可以直接复制,完全不用进入复制模式。
