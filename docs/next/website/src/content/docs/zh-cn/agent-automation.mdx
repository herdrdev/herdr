...
成功的 `agent wait` 返回窗格当前的状态,因此它可能与满足等待条件的状态不同。不同时,响应还会在 `.result.matched_transient_status` 中给出触发等待完成的状态。用 `.result.agent.agent_status` 判断的脚本需要考虑这一点。`agent prompt --wait` 不会在瞬时状态上完成,因此不会返回该字段。
