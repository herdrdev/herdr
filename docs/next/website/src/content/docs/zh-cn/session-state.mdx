...
启动时会记录带有会话文件路径的 `persist.restore` 结果,包括文件缺失、读取或解析错误、版本不受支持,以及空会话或恢复不完整的情况。即使所有窗格都保留下来,只要有窗格因保存的 cwd 不再存在而在备用目录中恢复,结果也会标记为 `partial`。

启动后首次保存或清空会话前,Herdr 会把已有的 `session.json` 备份到旁边的 `session-backups/` 目录。如果备份失败,原文件会保持不变。后续自动保存和重启不会覆盖之前的备份。备份仅包含会话快照,不包含 `session-history.json`,并会保留到你手动删除为止。

`persist.backup` 日志会给出恢复文件的路径。恢复时,先停止对应的服务器,用选定的备份替换它的 `session.json`,然后重新启动。Herdr 不会自动恢复备份,因此主动关闭的工作区不会重新出现。
...
普通的 `herdr update` 会安装新客户端,并让端点第 1 代服务器继续运行。普通的 `herdr --remote workbox` 也会在版本不同时保留兼容的远程服务器。只有从早于第 1 代的服务器做一次性升级时才需要停止;如果要在不结束窗格进程的情况下明确替换受支持的运行中服务器,请使用 `--handoff`。
