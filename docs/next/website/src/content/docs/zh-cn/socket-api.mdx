...
成功的 `agent.wait` 响应始终在 `agent.agent_status` 中保留窗格当前探测到的状态。如果某个瞬时状态满足了等待条件,但在最终探测前发生变化,响应还会包含 `matched_transient_status`。该字段表示触发等待完成的状态,不是窗格的当前状态。
...
对于进程内运行的官方集成,Herdr 会将上报绑定到该窗格中第一个上报的进程。只要该进程仍控制窗格,其他进程就不能替换所有者;两种上报方法都会返回错误码 `agent_report_refused`。拥有该窗格的智能体进程退出时,请调用 `pane.release_agent`。外部编排器应使用不绑定进程的 `custom:*` 来源。
