...
| 工作区分组 | `workspace_group.list`、`workspace_group.set`、`workspace_group.rename`、`workspace_group.delete` |
...
`workspace_group.set` 将工作区分配到具名分组;名称是新的时会创建分组,省略 `group` 则清除成员关系。对 worktree 空间任一成员的分配会应用到整个空间,失去全部成员的分组会被自动删除。`workspace_group.rename` 和 `workspace_group.delete` 通过 `workspace_group.list` 返回的稳定 `group_key` 定位分组;工作区响应包含可选的 `group` 字段描述成员关系。
