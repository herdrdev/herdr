...
拖动侧边栏右侧分隔线可显示较长的 Agent 和 Space 详细信息。默认调整范围现在为 18～80 列，仍可通过 `sidebar_min_width` 和 `sidebar_max_width` 配置。右键单击完整的 Agent 或 Space 行或顶部标签页并选择 **Color**，即可从十二种预设颜色中选择，或输入严格的 `#RGB`/`#RRGGBB` 值。Herdr 会自动为彩色项目的所有文本使用清晰可读的黑色或白色；选择 **clear color** 可恢复所有默认主题样式。这些显示颜色会在对应 Herdr 服务器运行期间保留，并在服务器彻底重启后清除。
