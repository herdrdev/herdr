...
拖动侧边栏右侧分隔线可显示较长的 Agent 和 Space 详细信息。默认调整范围现在为 18～80 列，仍可通过 `sidebar_min_width` 和 `sidebar_max_width` 配置。右键单击完整的 Agent 或 Space 行或顶部标签页并选择 **Color**，即可从十二种预设颜色中选择，或输入严格的 `#RGB`/`#RRGGBB` 值。Herdr 会自动为彩色项目的所有文本使用清晰可读的黑色或白色；选择 **clear color** 可恢复所有默认主题样式。这些显示颜色会在对应 Herdr 服务器运行期间保留，并在服务器彻底重启后清除。在 Agent 行上选择 **Unread** 可将 idle 智能体恢复为 `done`；该标记会一直保留，直到对应窗格本身获得焦点。

展开的桌面侧边栏还包含一个紧凑的 **Health Check** 区域，显示运行 Herdr 服务器的主机 CPU 使用率、可用时的电池百分比、RAM 已用量/总量和磁盘已用量/总量。上下拖动 Health Check 上方的分隔线即可调整区域大小，所选位置会随会话保存。因此，远程连接显示的是远程主机数据。在没有可读电池的台式机、容器或其他系统上，电池会显示为 `n/a`。
...
- `workspace` — 工作区名称。
- `tab` — 标签页名称，包括自动生成的数字名称。
- `pane` — 窗格名称（如有）。
- `pane_or_tab` — 优先显示窗格名称；没有窗格名称时显示标签页名称。
- `directory` — 启动智能体时所在窗格工作目录的基本名称。
...
token 会按配置顺序渲染。Herdr 使用 ` · ` 分隔相邻值，并在 `state_icon` 后使用一个空格。缺失值及其分隔符会消失；当一行中的所有 token 都没有值时，该行会消失。每个布局最多可包含 16 行，每行最多可包含 16 个 token。
