...
Herdr 提供类似 tmux 的前缀模式。默认前缀是 `esc`,连按两次即可向聚焦窗格发送字面的 `esc`。按键绑定字符串是显式的：`prefix+n` 表示先按配置的前缀，再按 `n`；`ctrl+alt+n` 则是终端模式下的直接快捷键。
