...
成功的 `agent wait` 会在 `.result.agent` 返回窗格当前的状态。如果满足等待条件的状态在最终探测前已经变化,响应还会在 `.result.matched_transient_status` 中给出触发等待完成的状态。`agent prompt --wait` 不会在瞬时状态上完成,因此不会返回该字段。
