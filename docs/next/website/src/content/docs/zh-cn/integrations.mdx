---
title: 集成
description: 为 Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Cursor Agent CLI、MastraCode 和 Codewhale 安装 Herdr 集成。
---

Herdr 自动检测受支持的智能体。官方集成可以额外提供用于恢复的原生会话身份、生命周期状态上报,或两者兼有。

当你想要 Claude Code/Codex/Copilot/Devin 式钩子的智能体原生会话恢复、Pi/OMP/Kimi/OpenCode/Kilo/Hermes/MastraCode/Codewhale 式钩子或插件的直接生命周期上报,或两者都要时,使用集成。完整的状态权威模型见[智能体](/zh-cn/docs/agents/)。
...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、MastraCode、Codewhale | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
该钩子向 Herdr 上报 Codewhale 生命周期状态和会话身份,用于权威的 `idle`、`working`、`blocked` 状态和原生恢复。Codewhale 没有屏幕清单兜底;当 Codewhale 在 Herdr 窗格内运行时,状态来自该钩子。

Herdr 使用 `~/.codewhale`（设置时使用 `CODEWHALE_CONFIG_DIR`）。安装会写入 `hooks/herdr-agent-state.sh`,并在 `config.toml` 的 `[hooks]` 下注册钩子条目,目录或配置文件不存在时会创建。卸载会删除由 Herdr 管理的配置块并删除钩子脚本。

Herdr 用 `codewhale --resume <id>` 恢复保存的 Codewhale 会话。
