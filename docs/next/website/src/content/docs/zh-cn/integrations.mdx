...
Herdr 默认使用 `~/.grok`,设置了 `GROK_HOME` 时使用后者。Grok 配置目录必须已经存在。Grok 会合并该目录中的每个 `hooks/*.json` 文件,所以安装会在 `hooks/herdr-agent-state.sh` 脚本旁写入一个自包含的 `hooks/herdr.json`,其中包含 Herdr 的 `SessionStart` 条目,并且不会编辑其他钩子文件。卸载只会删除这两个 Herdr 拥有的文件。

在 Grok 发出会话启动事件后,Herdr 可以用上报的会话 id 通过 `grok --resume <id>` 恢复该窗格。
