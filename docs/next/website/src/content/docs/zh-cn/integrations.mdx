---
title: 集成
description: 为 Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 和 Jcode 安装 Herdr 集成。
---
...
| 会话身份 | Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI、Jcode | 集成上报用于恢复的原生会话引用。状态仍来自 Herdr 的屏幕清单检测。 |
...
一些集成会上报智能体的原生会话引用。除非被 `[session] resume_agents_on_restore = false` 禁用,Herdr 会在服务器重启后使用官方会话引用恢复 Claude Code、Codex、Devin CLI、Droid、Kimi Code CLI、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、Grok CLI、Jcode、GitHub Copilot CLI、Pi、OMP、Hermes Agent、OpenCode、Kilo Code CLI、MastraCode 和 Antigravity CLI 的窗格。

原生会话恢复需要最新的 Herdr 集成: Pi 集成版本 `2`、OMP 版本 `3`、Claude Code 版本 `6`、Codex 版本 `5`、GitHub Copilot CLI 版本 `2`、Devin CLI 版本 `2`、Droid 版本 `2`、Kimi Code CLI 版本 `3`、Qoder CLI 版本 `2`、Qwen Code 版本 `1`、Letta Code 版本 `1`、Cursor Agent CLI 版本 `1`、Grok CLI 版本 `2`、Jcode 版本 `1`、OpenCode 版本 `5`、Kilo Code CLI 版本 `1`、Hermes Agent 版本 `5`、MastraCode 版本 `1`、Antigravity CLI 版本 `1`。用 `herdr integration status` 查看已安装版本。
...
Jcode 集成的安装是实验性功能,仅可通过 CLI 使用。请使用 `herdr integration install jcode`、`herdr integration uninstall jcode` 和 `herdr integration status` 管理它。它不会列在设置的集成界面或 JSON 集成 API 中。状态会将其显示为 `jcode (experimental)`。智能体检测、智能体启动和提示词支持仍可通过动态智能体 kind 独立使用。
...
当 Jcode 在 Herdr 窗格中运行时,该钩子通过 Jcode 的 `session_start` 观察器上报原生会话身份。Jcode 的状态仍由屏幕内容决定:输入区标记和处理状态行提供 working 与 idle 的判断依据,而 shell 模式和用户打开的浮层保持 idle。

该集成需要 Jcode `0.68.0` 或更高版本,这些版本支持生命周期钩子命令数组。

设置了 `JCODE_HOME` 时,Herdr 使用该目录,否则使用 `~/.jcode`。Jcode 配置目录必须已经存在。在 Unix 上,安装会写入 `hooks/herdr-agent-state.sh`,该脚本需要 `bash` 和 `python3`。在 Windows 上,安装会写入 `hooks/herdr-agent-state.ps1`,该脚本使用 PowerShell 和原生 Herdr CLI。

安装会将平台对应的钩子命令追加到 `config.toml` 的 `[hooks].session_start` 中。Jcode 会直接运行该生命周期钩子数组中的每条命令。重新安装不会重复添加 Herdr 命令,卸载只会移除 Herdr 命令,保留所有其他观察器。

Jcode 发出会话启动事件后,只有在 Jcode 对话记录已保存的情况下,Herdr 才能在服务器重启后通过 `jcode --resume <id>` 恢复窗格。刚清空的空会话在提示词触发保存之前不会持久化。该集成只上报会话身份,从不上报生命周期状态。
