---
title: 集成
description: 为 Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 和 Cline CLI 安装 Herdr 集成。
---
...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Cline CLI | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
这些钩子会向 Herdr 上报 Cline 的会话身份和生命周期状态,实现原生恢复以及权威的 `idle` 和 `working` 状态。可见的审批提示仍来自 Herdr 的屏幕清单检测。

Herdr 默认使用 `~/.cline`,设置了 `CLINE_DIR` 时使用后者。Cline 配置目录必须已经存在。Cline 按 `hooks/` 目录中的文件名发现钩子文件,因此安装会写入一个上报脚本,并为每个 Cline 生命周期事件写入一个小的包装文件(`taskstart.sh`、`taskresume.sh`、`userpromptsubmit.sh`、`pretooluse.sh`、`posttooluse.sh`、`taskcomplete.sh`、`taskcancel.sh`、`taskerror.sh`)。卸载只删除由 Herdr 管理的文件,不会触碰该目录中的其他钩子。

Cline 集成目前不支持 Windows,因为 Cline 通过 `bash`/`node` 运行器分发钩子文件,而这些运行器在 Windows 上不一定存在。没有钩子时屏幕检测仍然有效。

Herdr 可以用 `cline --id <id>` 恢复已保存的 Cline 会话。
