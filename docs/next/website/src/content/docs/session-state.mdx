...
Startup logs a `persist.restore` result with the session file path, including missing files, read or parse errors, unsupported versions, and empty or incomplete restores.
