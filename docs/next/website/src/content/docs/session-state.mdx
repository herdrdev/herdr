...
To inject extra CLI flags into the hardcoded resume argv (for example Claude bypass-permissions or Codex yolo), set per-agent args under `[session.resume_extra_args]`. Args are inserted immediately after the executable name:
