...
Startup logs a `persist.restore` result with the session file path, including missing files, read or parse errors, unsupported versions, and empty or incomplete restores. The `partial` outcome includes panes restored in a fallback directory because their saved cwd no longer exists, even when all panes survive.

Before the first save or clear after startup, Herdr preserves an existing `session.json` in the adjacent `session-backups/` directory. If that backup fails, Herdr leaves the original file untouched. Later autosaves and restarts do not overwrite previous backups. Backups contain the session snapshot, not `session-history.json`, and remain until you remove them.

The `persist.backup` log entry gives the recovery file path. To recover it, stop the affected server, copy the chosen backup over its `session.json`, then start it again. Herdr does not automatically restore backups, so intentionally closed workspaces stay closed.
