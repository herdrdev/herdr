...
Show the current visual index alongside custom tab names with:
...
The index follows the tab's current position, so it updates after tabs are reordered. Unnamed tabs already display their visual index and are not repeated.
