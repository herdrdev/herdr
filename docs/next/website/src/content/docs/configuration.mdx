...
Set `sidebar_position = "right"` under `[ui]` to place the desktop sidebar on the right. The default is `"left"`. This applies to expanded and compact sidebars; mobile layout is unchanged. Reload config to apply it.
