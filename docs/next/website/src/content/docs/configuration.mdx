...
Herdr has a navigation mode reached through a prefix key, similar to tmux. The default prefix is `ctrl+s`. Press `enter` to leave navigation mode where you landed, `esc` to leave it and return to the pane you started on, or the prefix key again to leave and send a literal `ctrl+s` to the focused pane. Keybinding strings are explicit: `prefix+n` means press the configured prefix and then `n`; `ctrl+alt+n` is a direct terminal-mode shortcut.
...
The default keymap is prefix-first so Herdr does not steal input from shells, editors, tmux, or terminal apps. Search `keys.` in the [Config reference](/docs/config-reference/) to see every action and default binding. The command palette at `prefix+space` shows the active bindings and rebinds them inline.

A binding may also be an array when one action needs multiple shortcuts:

```toml
[keys]
next_tab = ["prefix+shift+a", "ctrl+alt+]"]
```
...
Key strings accept plain keys, modifier combinations such as `ctrl+a`, `shift+n`, `alt+1`, `cmd+k`, and special keys such as `enter`, `tab`, `esc`, `left`, `right`, `up`, and `down`. Named punctuation such as `minus`, `comma`, `ampersand`, `plus`, and `backtick` is also accepted. Plain direct printable keys such as `n` are unsafe because they intercept typing; use `prefix+n` unless you intentionally want a direct binding. The `navigate_workspace_*`, `navigate_tab_*`, and `navigate_pane_*` fields are navigate-mode-only and may use plain keys such as `n` or `e`; they must not use `prefix+`, `esc`, `enter`, `tab`, `shift+tab`, or unmodified `1` through `9`. Up and down switch workspaces, left and right switch tabs, and `n`/`e`/`i`/`a` move between panes, none of which leave navigation mode, so they can be pressed repeatedly. Left and right fall back to pane-left and pane-right navigation when `navigate_tab_previous` and `navigate_tab_next` are unbound. These navigate-mode shortcuts are independent from general action bindings such as `focus_pane_down = "prefix+e"`, which the navigate-mode set always shadows while navigate mode is open; bind `focus_pane_*` to a direct chord if you want pane focus without the prefix. Alt, Cmd/Super, and punctuation with modifiers depend on your terminal and tmux settings.
...
An optional `description` can be provided. When specified, this description is displayed in the command palette (opened with `prefix+space`) in place of the default `'custom command'` label.
