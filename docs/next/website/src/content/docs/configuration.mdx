...
Drag the sidebar's right divider to reveal longer Agent and Space details. The default resize range is now 18–80 columns and remains configurable with `sidebar_min_width` and `sidebar_max_width`. Right-click a complete Agent or Space row, or a top tab, and choose **Color** to apply one of 12 presets or enter a strict `#RGB`/`#RRGGBB` value. Herdr automatically uses readable black or white text across the colored item; **clear color** restores every default theme style. These presentation colors remain available while that Herdr server is running and clear after a cold server restart.
