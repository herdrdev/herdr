...
The symbols are static, so this option does not enable spinner animation. To animate working agents in the desktop sidebar, opt in explicitly:
...
Animated indicators use `▲` for blocked, an 80 ms braille spinner for working, `✓` for done and idle, and `·` for unknown. Animation stops outside the desktop sidebar or when no connected agent is working. Because each frame writes terminal output, it can affect host-terminal cursor blinking and copy-mode snapshots; `dots` remains the default.
