...
Drag the sidebar's right divider to reveal longer Agent and Space details. The default resize range is now 18–80 columns and remains configurable with `sidebar_min_width` and `sidebar_max_width`. Right-click a complete Agent or Space row, or a top tab, and choose **Color** to apply one of 12 presets or enter a strict `#RGB`/`#RRGGBB` value. Herdr automatically uses readable black or white text across the colored item; **clear color** restores every default theme style. These presentation colors remain available while that Herdr server is running and clear after a cold server restart. On an Agent row, choose **Unread** to return an idle agent to `done`; the marker remains until that exact pane receives focus.

The expanded desktop sidebar also includes a compact **Health Check** section. It shows CPU usage, battery percentage when a battery is present, RAM used/total, and disk used/total. Drag the divider above Health Check vertically to resize the section; the chosen position persists with the session. These values are sampled from the host running the Herdr server, so a remote attach reports the remote host. Battery displays `n/a` on desktops, containers, and other systems without a readable battery.
...
- `tab` — tab name, including its automatic numeric name.
- `pane` — pane name when available.
- `pane_or_tab` — pane name when available, otherwise the tab name.
- `directory` — base name of the pane working directory from which the agent was launched.
...
Tokens render in their configured order. Herdr separates adjacent values with ` · ` and uses a single space after `state_icon`. Missing values and their separators disappear; a row disappears when none of its tokens have a value. Each layout may contain at most 16 rows, with at most 16 tokens in each row.
