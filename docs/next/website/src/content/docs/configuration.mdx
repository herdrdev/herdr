...
Herdr has a prefix mode similar to tmux. The default prefix is `esc`; press it twice to send a literal `esc` to the focused pane. Keybinding strings are explicit: `prefix+n` means press the configured prefix and then `n`; `ctrl+alt+n` is a direct terminal-mode shortcut.
