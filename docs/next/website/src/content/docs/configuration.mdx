...
Optional per-agent extra CLI arguments are inserted after the agent executable on resume, so global flags apply before subcommands:
...
That turns restore into `claude --dangerously-skip-permissions --resume <id>` and `codex --yolo resume <id>`. Keys must be strict canonical agent ids. Changes apply on the next Herdr server restart restore, not to agents already running.
