...
Detach the client with `ctrl+s q`. Your panes keep running.
...
This works like a terminal multiplexer. Your shell is remote. The Herdr server is remote. The agents and panes run on the remote machine. Detach with `ctrl+s q`, disconnect, then SSH back and run `herdr` again.
