...
Detach the client with `ctrl+s q`; panes and agents keep running. Reattach by running `herdr` again. Stop the session and its panes with `herdr server stop`.
...
Detach with `ctrl+s q`. Send a literal `ctrl+s` with `ctrl+s ctrl+s`.
