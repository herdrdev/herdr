...
With a server that advertises surface activation support, the client reconnects after a transport failure and restores the remote surface. Panes keep running on the server. Input typed while disconnected is discarded, and unconfirmed input is not replayed. Explicitly stopping the server still ends the attach. Reconnection begins after the failure is detected; servers advertising health checks also receive periodic probes. Older compatible servers remain attachable without automatic reconnect.

For optional immediate echo while typing, enable `[remote].predict_input = true` in your local config. See [Remote attach configuration](/docs/configuration/#remote-attach) for its supported input and limitations. This uses the existing SSH connection and does not require Mosh, UDP ports, or a server upgrade. It does not provide Mosh's transport roaming or avoid TCP stalls during packet loss.
