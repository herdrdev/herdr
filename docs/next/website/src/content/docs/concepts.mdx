...
Detach the client with `ctrl+s q`. The server and agents continue running.
...
Herdr has terminal mode and navigation mode.

Terminal mode sends keys to the focused pane. Navigation mode is the command surface: keys run Herdr actions, the sidebar takes the workspace selection, and the mode stays open until you leave it.

Press the prefix key, default `ctrl+s`, then an action key such as `t` for a new tab or `space` for the command palette. Press `enter` to leave navigation mode where you landed, `esc` to return to the pane you started on, or the prefix key again to leave and send a literal prefix to the pane. New to the prefix idea? See [Keyboard](/docs/keyboard/).
