...
Detach the client with `esc q`. The server and agents continue running.
...
Press the prefix key, default `esc`, then an action key such as `c` for a new tab or `space` for the command palette. Press the prefix key again to return to the terminal. New to the prefix idea? See [Keyboard](/docs/keyboard/).
