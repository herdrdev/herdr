...
成功した `agent wait` はペインの現在の状態を返すため、待機条件を満たした状態とは異なる場合があります。異なる場合、レスポンスには待機を完了させた状態が `.result.matched_transient_status` にも含まれます。`.result.agent.agent_status` で判定するスクリプトは、この点を考慮してください。`agent prompt --wait` は一時的な状態で解決することがないため、このフィールドを返しません。
