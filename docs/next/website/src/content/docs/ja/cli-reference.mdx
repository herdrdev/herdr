...
タスクはサーバーに保存される永続レコードです。`task attach` と `task dispatch` は Herdr で実行中のエージェント名またはペイン ID を受け取り、Herdr 外で起動したプロセスは取り込みません。`--agent-session-id` は既知の Codex、Claude、Pi、またはその他のエージェントのネイティブセッション ID をタスクメタデータとして記録しますが、そのセッションをアタッチまたは再開するものではありません。実行中のペインを別のワークスペースへ移すには `pane move` を使います。
