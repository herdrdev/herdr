...
成功した `agent wait` は、ペインの現在の状態を `.result.agent` に返します。待機条件を満たした状態が最終検査までに変化していた場合、レスポンスには待機を完了させた状態が `.result.matched_transient_status` にも含まれます。`agent prompt --wait` は一時的な状態で解決することがないため、このフィールドを返しません。
