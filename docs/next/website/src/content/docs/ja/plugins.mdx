...
Windows では、各ビルドコマンドとその子孫プロセスはインストーラーが管理する
ジョブ内で実行されます。ビルドコマンドが終了すると、Herdr は残っている子孫
プロセスを終了してからチェックアウトを移動または削除します。ビルドコマンドは
終了前に処理を完了してください。ランタイムプロセスはプラグインのアクションや
スタートアップフックから起動してください。
