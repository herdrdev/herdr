...
`Background` と表示された場合は、サーバーを停止し、通常の GUI ターミナルから Herdr を再起動してください:
...
## コピーが「copy sent to terminal」または「copy failed」と表示される

コピーのポップアップは、コピーを試みたことではなく、テキストが実際にどのように
配送されたかを報告します。

`copied to clipboard` は、ネイティブのクリップボードツールが書き込みを確認したこと
を意味します。`copy sent to terminal` は、代わりに Herdr が外側のターミナルへ
OSC 52 エスケープを送出したことを意味します。これはターミナルが応答を返さない
一方向の書き込みなので、ターミナルが OSC 52 に対応している場合にのみ機能します。
`copy failed` は何も配送されなかったことを意味します。

Herdr は、ネイティブのクリップボードツールが利用できない場合、および SSH 経由、
WSL 内、VS Code の統合ターミナル内で OSC 52 を使います。これらの環境では、サーバー
側のツールは意図しないマシンのクリップボードに書き込んでしまうためです。ローカルで
確認済みのコピーを行うには、そのホストにクリップボードツールを導入してください。
Linux では `wl-clipboard`、`xclip`、`xsel`（macOS には `pbcopy` が同梱されています）。
OSC 52 を使い続ける場合は、外側のターミナルで OSC 52 を有効にするか、対応している
ターミナルを使ってください。もう一つの方法は `herdr --remote <host>` です。この場合
クライアントは手元のマシンで動くため、ローカルのクリップボードに直接書き込みます。
