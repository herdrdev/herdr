---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI、Jcode 向けの Herdr インテグレーションをインストールします。
---
...
| セッション識別 | Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI、Jcode | インテグレーションは復元用のネイティブセッション参照を報告します。状態は引き続き Herdr のスクリーンマニフェスト検出から得られます。 |
...
Jcode インテグレーションのインストールは実験的機能で、CLI からのみ利用できます。管理には `herdr integration install jcode`、`herdr integration uninstall jcode`、`herdr integration status` を使います。設定のインテグレーション画面や JSON インテグレーション API には表示されません。ステータスでは `jcode (experimental)` と表示されます。エージェント検出、エージェントの起動、プロンプトのサポートは、動的なエージェント kind を通じて独立して利用できます。
...
このフックは、Jcode が Herdr ペイン内で動作している間、Jcode の `session_start` オブザーバーを通じてネイティブセッションの識別情報を報告します。Jcode の状態は引き続き画面から判断されます。入力欄のマーカーと処理ステータス行が working と idle の判断材料となり、シェルモードやユーザーが開いたオーバーレイは idle のままです。

このインテグレーションには、ライフサイクルフックのコマンド配列に対応した Jcode `0.68.0` 以降が必要です。

Herdr は `JCODE_HOME` が設定されていればそれを使い、そうでなければ `~/.jcode` を使います。Jcode の設定ディレクトリはあらかじめ存在している必要があります。Unix では、インストール時に `hooks/herdr-agent-state.sh` を書き込みます。このスクリプトには `bash` と `python3` が必要です。Windows では、PowerShell とネイティブの Herdr CLI を使う `hooks/herdr-agent-state.ps1` を書き込みます。

インストールは、プラットフォームに対応したフックコマンドを `config.toml` の `[hooks].session_start` に追加します。Jcode は、このライフサイクルフック配列内の各コマンドを直接実行します。再インストールしても Herdr コマンドは重複せず、アンインストールではほかのすべてのオブザーバーを保持したまま Herdr コマンドだけを削除します。

Jcode がセッション開始イベントを発行した後、Herdr は Jcode の会話履歴が保存されている場合に限り、サーバーの再起動後に `jcode --resume <id>` でペインを resume できます。クリア直後の空のセッションは、プロンプトによって保存されるまで永続化されません。このインテグレーションはセッションの識別情報だけを報告し、ライフサイクル状態は一切報告しません。
