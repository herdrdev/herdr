...
Herdr はデフォルトで `~/.grok` を使い、`GROK_HOME` が設定されていればそちらを使います。Grok の設定ディレクトリはあらかじめ存在している必要があります。Grok はそのディレクトリ内のすべての `hooks/*.json` ファイルをマージするため、インストールは Herdr の `SessionStart` エントリを含む自己完結した `hooks/herdr.json` を `hooks/herdr-agent-state.sh` スクリプトの隣に書き込み、他のフックファイルは編集しません。アンインストールは Herdr が所有するその 2 つのファイルだけを削除します。
