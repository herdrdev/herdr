---
title: インテグレーション
description: Pi、OMP、Veyyon、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Veyyon、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
Veyyon インテグレーションをインストールします:

```bash
herdr integration install veyyon
```

Herdr は組み込み拡張を `~/.veyyon/profiles/default/agent/extensions/herdr-veyyon-agent-state.ts` に書き込みます。`VEYYON_CODING_AGENT_DIR` は agent ディレクトリ全体を上書きします。それ以外では `VEYYON_CONFIG_DIR` が設定ルートを、`VEYYON_PROFILE` が profile を選択します。解決された Veyyon agent ディレクトリが存在する場合、Herdr は extensions ディレクトリを作成します。アンインストールはその拡張ファイルだけを削除します。

この拡張は Veyyon が Pi/OMP から継承したライフサイクルイベントを再利用し、Herdr のソケット API を通じて `working`、`blocked`、`idle` とネイティブセッション識別を報告します。拡張がない場合は Veyyon のスクリーンマニフェストへフォールバックします。Herdr は保存済みセッションを `veyyon --resume <session>` で復元します。
