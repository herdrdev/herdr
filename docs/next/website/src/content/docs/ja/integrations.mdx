---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI、Cline CLI 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Cline CLI | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
Cline CLI フックをインストールします:

```bash
herdr integration install cline
```

このフックは、Cline のセッション識別とライフサイクル状態を Herdr に報告し、ネイティブな復元と `idle`・`working` の権威あるステータスを実現します。表示上の承認プロンプトは引き続き Herdr のスクリーンマニフェスト検出から得られます。

Herdr はデフォルトで `~/.cline` を使い、`CLINE_DIR` が設定されていればそちらを使います。Cline の設定ディレクトリはあらかじめ存在している必要があります。Cline は `hooks/` 内のファイル名でフックファイルを発見するため、インストールはレポータースクリプトと、各 Cline ライフサイクルイベントごとの小さなラッパーファイル（`taskstart.sh`、`taskresume.sh`、`userpromptsubmit.sh`、`pretooluse.sh`、`posttooluse.sh`、`taskcomplete.sh`、`taskcancel.sh`、`taskerror.sh`）を書き込みます。アンインストールは Herdr 所有のファイルだけを削除し、そのディレクトリ内のほかのフックには触れません。

Cline インテグレーションは現在 Windows 非対応です。Cline がフックファイルを `bash` / `node` ランナー経由で実行するためで、これらが Windows 上に存在するとは限らないためです。フックがなくてもスクリーン検出は機能します。

Herdr は保存された Cline セッションを `cline --id <id>` で resume できます。
