---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI、Muse 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Muse | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
| セッション識別 | Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI | インテグレーションは復元用のネイティブセッション参照を報告します。状態は引き続き Herdr のスクリーンマニフェスト検出から得られます。 |
...
このフックは、ネイティブ復元と権威ある `idle`、`working`、`blocked` 状態のために、Muse のセッション識別とライフサイクル状態を Herdr に報告します。

`XDG_CONFIG_HOME` が設定されていれば、Herdr は `$XDG_CONFIG_HOME/muse` を使います。それ以外では、Linux と macOS では `~/.config/muse`、Windows では `%APPDATA%\muse` を使います。Muse の設定ディレクトリはあらかじめ存在している必要があります。インストールは `hooks/herdr-agent-state.sh`（Windows では `hooks/herdr-agent-state.ps1`）を書き込み、ユーザー所有の `settings.json` に Herdr 管理のフックエントリをマージします（無関係なフックは保持します）。アンインストールは Herdr 管理のフックエントリを削除し、フックスクリプトを削除します。

Herdr は保存された Muse セッションを `muse resume <id>` で resume します。

| Muse フックイベント | Herdr での効果 |
| --- | --- |
| `SessionStart` | 復元用のネイティブセッション id を報告します。セッションごとに 1 回発行されます。 |
| `UserPromptSubmit` | `working` を報告します。 |
| `PreToolUse` | `working` を報告します。 |
| `PostToolUse` | `working` を報告します。 |
| `PermissionRequest` | `blocked` を報告します。 |
| `Stop` | `idle` を報告します。 |
| `Interrupt` | `idle` を報告します。 |
| `SessionEnd` | セッション報告を終了します。ライフサイクルの解放はプロセス終了に従います。 |
