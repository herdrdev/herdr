...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、MastraCode、Codewhale | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
Codewhale フックをインストールします:

```bash
herdr integration install codewhale
```

このフックは、Codewhale のライフサイクル状態とセッション識別を Herdr に報告し、権威ある `idle`、`working`、`blocked` 状態とネイティブ復元を提供します。Codewhale にはスクリーンマニフェストのフォールバックはありません。Codewhale が Herdr ペイン内で動いている間、状態はフックから得られます。

Herdr は `~/.codewhale`（設定されている場合は `CODEWHALE_CONFIG_DIR`）を使います。インストールは `hooks/herdr-agent-state.sh` を書き込み、`config.toml` の `[hooks]` 配下にフックエントリを登録します（ディレクトリや設定ファイルが存在しない場合は作成します）。アンインストールは Herdr 管理の設定ブロックを削除し、フックスクリプトを削除します。

Herdr は保存された Codewhale セッションを `codewhale --resume <id>` で resume します。
