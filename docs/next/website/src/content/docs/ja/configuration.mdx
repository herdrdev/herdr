...
Herdr には tmux に似たプレフィックスモードがあります。デフォルトのプレフィックスは `esc` で、2 回押すとフォーカス中のペインにリテラルの `esc` を送ります。キーバインド文字列は明示的です。`prefix+n` は設定されたプレフィックスを押してから `n` を押すという意味で、`ctrl+alt+n` はターミナルモードの直接ショートカットです。
