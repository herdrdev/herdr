...
サイドバー右側の区切り線をドラッグすると、長い Agent と Space の詳細を表示できます。デフォルトのサイズ変更範囲は 18～80 列で、`sidebar_min_width` と `sidebar_max_width` で引き続き設定できます。Agent または Space の行全体、または上部タブを右クリックして **Color** を選ぶと、12 色のプリセットから選択するか、厳密な `#RGB`/`#RRGGBB` 値を入力できます。Herdr は色付き項目の全テキストに読みやすい黒または白を自動的に使用し、**clear color** ですべてのデフォルトテーマスタイルに戻せます。これらの表示色は Herdr サーバーの実行中は保持され、サーバーを完全に再起動するとクリアされます。
