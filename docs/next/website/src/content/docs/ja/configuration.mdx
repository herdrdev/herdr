...
サイドバー右側の区切り線をドラッグすると、長い Agent と Space の詳細を表示できます。デフォルトのサイズ変更範囲は 18～80 列で、`sidebar_min_width` と `sidebar_max_width` で引き続き設定できます。Agent または Space の行全体、または上部タブを右クリックして **Color** を選ぶと、12 色のプリセットから選択するか、厳密な `#RGB`/`#RRGGBB` 値を入力できます。Herdr は色付き項目の全テキストに読みやすい黒または白を自動的に使用し、**clear color** ですべてのデフォルトテーマスタイルに戻せます。これらの表示色は Herdr サーバーの実行中は保持され、サーバーを完全に再起動するとクリアされます。Agent 行で **Unread** を選ぶと idle のエージェントを `done` に戻せ、そのペイン自体がフォーカスされるまでマーカーが残ります。

展開されたデスクトップサイドバーには、コンパクトな **Health Check** セクションも表示されます。Herdr サーバーを実行しているホストの CPU 使用率、バッテリー残量（利用可能な場合）、RAM の使用量/総量、ディスクの使用量/総量を表示します。Health Check の上にある区切り線を上下にドラッグしてセクションのサイズを変更でき、選択した位置はセッションに保存されます。そのため、リモート接続ではリモートホストの値が表示されます。読み取り可能なバッテリーがないデスクトップやコンテナでは、バッテリーは `n/a` と表示されます。
...
- `tab` — 自動生成された番号を含むタブ名。
- `pane` — 利用可能な場合はペイン名。
- `pane_or_tab` — 利用可能な場合はペイン名。それ以外はタブ名。
- `directory` — エージェントを起動したペインの作業ディレクトリのベース名。
...
トークンは設定された順序で描画されます。Herdr は隣接する値を ` · ` で区切り、`state_icon` の後には空白を 1 つ入れます。値がない場合、その値と区切りは表示されません。すべてのトークンに値がない行は表示されません。各レイアウトは最大 16 行、各行は最大 16 トークンです。
