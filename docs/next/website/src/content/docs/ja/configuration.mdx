...
色の値には、hex、名前付きの色、`rgb(r,g,b)`、`0` から `255` の ANSI パレットインデックス、または `reset`、`default`、`none`、`transparent` のようなリセットエイリアスが使えます。
...
起動時の競合を減らすため、自動復元ではデフォルトで 100 ミリ秒間隔でエージェントを 1 つずつ起動します。起動条件を満たした最初のエージェントはすぐに起動します。`startup_per_agent_delay_ms = 0` で間隔を無効にできます。手動起動と明示的なターミナルへの接続には影響しません。変更を適用するには Herdr サーバーを再起動してください。
