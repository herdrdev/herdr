...
Herdr には tmux と同じように、プレフィックスキーで入るナビゲーションモードがあります。デフォルトのプレフィックスは `ctrl+s` です。`enter` でたどり着いた場所のままナビゲーションモードを抜け、`esc` で開始時のペインに戻り、もう一度プレフィックスキーを押すと抜けると同時にフォーカス中のペインにリテラルの `ctrl+s` を送ります。キーバインド文字列は明示的です。`prefix+n` は設定されたプレフィックスを押してから `n` を押すという意味で、`ctrl+alt+n` はターミナルモードの直接ショートカットです。
...
デフォルトのキーマップはプレフィックス優先なので、Herdr がシェル、エディタ、tmux、ターミナルアプリから入力を奪うことはありません。[設定リファレンス](/docs/config-reference/)で `keys.` を検索すると、すべてのアクションとデフォルトのバインドを確認できます。`prefix+space` のコマンドパレットには現在有効なバインドが表示され、その場で割り当てを変更できます。
...
キー文字列には、通常のキー、`ctrl+a`、`shift+n`、`alt+1`、`cmd+k` のような修飾キーの組み合わせ、`enter`、`tab`、`esc`、`left`、`right`、`up`、`down` のような特殊キーが使えます。`minus`、`comma`、`ampersand`、`plus`、`backtick` のような名前付き記号も使えます。`n` のような通常の印字可能キーの直接バインドは入力を妨げるため危険です。意図的に直接バインドするのでなければ `prefix+n` を使ってください。`navigate_workspace_*`、`navigate_tab_*`、`navigate_pane_*` のフィールドはナビゲートモード専用で、`n` や `e` のような通常のキーを使えます。これらには `prefix+`、`esc`、`enter`、`tab`、`shift+tab`、修飾なしの `1` から `9` は使えません。上下キーはワークスペースを、左右キーはタブを、`n`/`e`/`i`/`a` はペインを切り替えます。いずれもナビゲーションモードを抜けないので、続けて押せます。`navigate_tab_previous` と `navigate_tab_next` が未割り当ての場合、左右キーは左ペインと右ペインへのナビゲーションにフォールバックします。これらのナビゲートモードショートカットは、`focus_pane_down = "prefix+e"` のような一般アクションのバインドから独立しており、ナビゲートモードが開いている間は常にナビゲートモード側が優先されます。プレフィックスなしでペインを移動したい場合は `focus_pane_*` に直接コードを割り当ててください。Alt、Cmd/Super、修飾キー付き記号は、ターミナルと tmux の設定に依存します。
...
任意で `description` を指定できます。指定すると、コマンドパレット（`prefix+space` で開く）に、デフォルトの `'custom command'` ラベルの代わりに表示されます。
