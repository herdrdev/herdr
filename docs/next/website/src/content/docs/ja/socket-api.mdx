...
| ワークスペースグループ | `workspace_group.list`、`workspace_group.set`、`workspace_group.rename`、`workspace_group.delete` |
...
`workspace_group.set` は、名前付きグループへワークスペースを割り当てます。名前が新規の場合はグループを作成し、`group` を省略するとメンバーシップを解除します。worktree スペースのメンバーへの割り当てはスペース全体に適用され、メンバーがいなくなったグループは自動的に削除されます。`workspace_group.rename` と `workspace_group.delete` は、`workspace_group.list` が返す安定した `group_key` でグループを指定します。ワークスペースのレスポンスには、メンバーシップを表す省略可能な `group` フィールドが含まれます。
