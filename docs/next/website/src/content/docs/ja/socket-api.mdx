...
成功した `agent.wait` レスポンスでは、ペインの現在の検査済み状態が常に `agent.agent_status` に保持されます。一時的な状態が待機条件を満たした後、最終検査までに変化した場合、レスポンスには `matched_transient_status` も含まれます。このフィールドは完了の契機となった状態であり、ペインの現在の状態ではありません。
...
プロセス内で動作する公式インテグレーションでは、Herdr は報告を、そのペインで最初に報告したプロセスに関連付けます。そのプロセスがペインを制御している間、別のプロセスは所有者を置き換えられません。どちらの報告メソッドも、コード `agent_report_refused` のエラーを返します。所有するエージェントプロセスの終了時には `pane.release_agent` を呼び出してください。外部オーケストレーターは、プロセスに関連付けられない `custom:*` ソースを使ってください。
