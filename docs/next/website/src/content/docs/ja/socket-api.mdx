...
`events_lost` を受け取ったら、新しい購読を開始して `subscription_started` を待ちます。`session.snapshot` の取得中は新しい購読のイベントをバッファし、スナップショットを適用してからバッファしたイベントを順番に適用し、そのまま読み取りを続けてください。読み取りが遅いと履歴の保持範囲を超えることがあるため、クライアントの読み取り速度を上げるか、イベントの負荷を減らす必要があります。
