...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.mark_unread`、`agent.start` |
...
`agent.mark_unread` は、他のエージェントメソッドと同じライブエージェント名またはホスト中のペイン ID を受け付けます。フォーカスを変えずに更新後のエージェントレコードを返し、idle のエージェントを `idle` から `done` に変更します。そのペイン自体がフォーカスされるまで `done` のままで、所属するワークスペースやタブ、または同じタブ内の別ペインをフォーカスしても表示済みにはなりません。
...
`session.snapshot` は、独自のローカルランタイムキャッシュを持つクライアント向けに、一度限りのブートストラップスナップショットを返します。レスポンスには、バージョン/プロトコルメタデータ、フォーカス中のワークスペース/タブ/ペイン id、ワークスペース、タブ、ペイン、タブレイアウト、エージェントの各レコード、および最新のサーバーホスト `health` サンプルが含まれます。`health` オブジェクトには `cpu_usage_percentage`、`battery`、`ram`、`disk` が含まれる場合があり、利用できないメトリクスは省略されます。`health` は特定時点の値なので、より新しいサンプルが必要なクライアントは `session.snapshot` を再度要求できます。その他のセッションリソースについては、取得後にリソースイベントを購読し、そのイベントでローカルキャッシュを更新してください。再接続後やキャッシュが古い可能性がある場合は、`session.snapshot` を再度呼び出します。ワークスペースレコードには、関連付けられた worktree の出自情報も含まれます。リポジトリ全体の worktree 検出には引き続き `worktree.list` を使います。
...
`pane.current` は単一の `PaneInfo` を返します。`caller_pane_id` があるときはそのペインを返します。省略時はアクティブなフォーカス中ペインを返します。`PaneInfo.label` は任意の手動ペインラベルで、`PaneInfo.display_label` は実際の表示名です。表示名がない場合は `pane 1` のようなタブ内の自動生成名になります。
...
既存タブへの移動には `split: "right" | "down"` が必要です。`target_pane_id` は任意で、デフォルトは移動先タブのフォーカス中ペインです。`type: "tab_placement"` 形式では対象ペインが必須で、同じタブまたは別のタブに対して `placement: "left" | "right" | "above" | "below"` を指定できます。任意の `moved_ratio` は常に移動するペインへ割り当てる比率です。従来の `type: "tab"` 形式は元のタブでは何も変更せず、`reason: "same_tab"` を返します。同じタブで明示的に入れ替えるには `pane.swap` を使います。ズームされた移動元/移動先タブを含む移動は `reason: "zoomed_tab"` 付きで `changed: false` を返します。
