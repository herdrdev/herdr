...
`idle` means the agent is ready for input and its pane has been seen in the focused Herdr UI. `done` is the same underlying idle state after background work finishes, until that exact pane is focused or `pane focus` / `agent focus` targets it. Focusing only the containing workspace, tab, or a sibling pane does not mark it seen. Reading through the CLI does not mark it seen. `blocked` means Herdr recognized an approval or question UI. `unknown` means an agent is present but Herdr cannot classify its lifecycle confidently; it does not prove successful completion. Use exact `--until` states when that distinction matters.
