...
A successful `agent wait` reports the pane's current status, which can differ from the status that satisfied the wait. When it differs, the response also carries the status that completed the wait at `.result.matched_transient_status`. Scripts that gate on `.result.agent.agent_status` must allow for that. `agent prompt --wait` never resolves on a transient status and never reports that field.
