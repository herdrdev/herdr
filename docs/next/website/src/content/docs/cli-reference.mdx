...
A successful `agent wait` reports the pane's current status in `.result.agent`. When the status that satisfied the wait had already changed by the final probe, the response also carries the status that completed the wait at `.result.matched_transient_status`. `agent prompt --wait` never resolves on a transient status and never reports that field.
