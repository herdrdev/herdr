...
The `--place` move form works within the same tab or across tabs and workspaces. It rebuilds the target split so the moved pane is on the requested side; `--ratio` is the share assigned to the moved pane. The older `--split` form remains available for cross-tab moves.
