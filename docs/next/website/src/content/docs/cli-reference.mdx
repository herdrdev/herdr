...
A successful start returns only after the expected agent owns the same terminal and is ready for interactive input. If detection reports `blocked` during startup, the command returns `agent_not_ready` immediately. The name remains available for `agent read` and `agent send-keys`. For agents with screen-based input detection, including OpenCode, prompts remain unavailable until the input controls are visible and no blocker is active; an `idle` report alone is insufficient. The default startup timeout is 30000 milliseconds; explicit values must be greater than 3000 and no more than 300000.

Cold-restored agents remain queued until their resume command is sent. They then pass the same startup readiness checks before accepting prompts; restoring a saved name or session does not itself make the agent ready.
