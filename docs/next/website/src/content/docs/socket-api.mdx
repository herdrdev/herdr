...
The successful `agent.wait` response always keeps the pane's current probed status in `agent.agent_status`. If a transient status satisfied the wait but changed before the final probe, the response also includes `matched_transient_status`. Treat that field as the status that triggered completion, not as the pane's current state.
...
For in-process official integrations, Herdr binds reports to the first reporting process for the pane. A different process cannot replace that owner while it still controls the pane: both report methods return an error with code `agent_report_refused`. Call `pane.release_agent` when the owning agent process exits. External orchestrators should use a `custom:*` source, which is not process-bound.
