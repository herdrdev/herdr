...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.mark_unread`, `agent.start`, `agent.view.set`, `agent.view.clear` |
...
`agent.mark_unread` accepts the same live agent name or hosting pane ID as other agent methods. It returns the updated agent record without changing focus and changes an idle agent from `idle` to `done`. The agent stays `done` until its exact pane is focused; focusing only its workspace, tab, or a sibling pane does not acknowledge it.
...
tab layout snapshots, agent records, and the latest server-host `health` sample.
The health object can contain `cpu_usage_percentage`, `battery`, `ram`, and
`disk`; unavailable metrics are omitted. Health is a point-in-time value, so
clients that need a fresher sample can request `session.snapshot` again. For
other session resources, subscribe to resource events and update the local
cache from those events. Call `session.snapshot` again after reconnecting or
when the local cache may be stale. Attached worktree provenance is included on
workspace records.
Full repo worktree discovery remains `worktree.list`.
...
pane. `PaneInfo.label` is the optional manual pane label, while
`PaneInfo.display_label` is the effective display name and falls back to the
tab-local generated name such as `pane 1`.
...
{"id":"req_place_above","method":"pane.move","params":{"pane_id":"w1:p2","destination":{"type":"tab_placement","tab_id":"w1:t2","target_pane_id":"w1:p3","placement":"above","moved_ratio":0.4},"focus":true}}
...
optional and defaults to the target tab's focused pane. The
`type: "tab_placement"` form requires a target pane and accepts
`placement: "left" | "right" | "above" | "below"` in the same tab or another
tab. Its optional `moved_ratio` is always the share assigned to the moved pane.
The older `type: "tab"` form remains a no-op in the source tab and returns
`reason: "same_tab"`; use `pane.swap` for an explicit same-tab swap. Moves
involving a zoomed source or target tab return `changed: false` with
`reason: "zoomed_tab"`.
