...
| Workspace group | `workspace_group.list`, `workspace_group.set`, `workspace_group.rename`, `workspace_group.delete` |
...
`workspace_group.set` assigns a workspace to a named group, creating the group when the name is new; omit `group` to clear membership. Assigning any member of a worktree space applies to the whole space, and groups left without members are dropped automatically. `workspace_group.rename` and `workspace_group.delete` address groups by the stable `group_key` returned from `workspace_group.list`; workspace responses include an optional `group` field describing membership.
