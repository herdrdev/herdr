...
On `events_lost`, open a new subscription and wait for `subscription_started`.
Buffer its events while requesting `session.snapshot`, install the snapshot,
then apply the buffered events and continue reading. Slow readers can still
exhaust the bounded history; increasing client read throughput or reducing
event load may be necessary.
