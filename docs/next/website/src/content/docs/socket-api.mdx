...
file-frame options, pixel-mouse support, and the 16-layer limit.

`pane.graphics.set`, `pane.graphics.clear`, and `pane.graphics.stream` accept an
optional `layer_id` (default `primary`) and `z_index`. Layers are placed in
stable `(z_index, layer_id)` order. Each stream exclusively owns its layer, and
