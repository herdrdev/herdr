...
| Action | Key |
| --- | --- |
| Move across non-empty status columns | `h/l` or Left/Right |
| Move between tasks in the current column | `j/k` or Up/Down |
| Open the selected task's activity | Enter |
| Cycle the task dock and close the board | `p` |
| Close the board | Esc or `q` |

See the [task-board manual](/docs/task-board/) for task automation, agent
dispatch, lifecycle reporting, and activity tracking.
