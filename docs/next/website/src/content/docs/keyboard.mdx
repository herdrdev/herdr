...
Navigation mode stays open until you leave it, so you can run several commands from one press. A hint line at the bottom of the screen lists what is bound. `space` opens the command palette, `↑`/`↓` move the workspace selection in the sidebar, `Enter` opens the selected workspace, and most commands that move focus drop you back in the terminal on their own.
...
| Workspace navigation surface | `prefix+w` |
| Toggle sidebar | `prefix+b` |

Digits belong to tabs, so jumping straight to a workspace needs its own binding, for example `switch_workspace = "prefix+shift+1..9"`. Inside navigation mode you can also pick a workspace with `↑`/`↓` and `Enter`.
