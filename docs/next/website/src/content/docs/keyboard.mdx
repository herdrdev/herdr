...
The prefix solves this. Press the prefix key, default `esc`, and the next keypress goes to Herdr instead of your terminal. `prefix+c` means: press `esc`, release, then press `c`. One reserved key instead of dozens.

With the default prefix, `esc esc` sends a literal `esc` to the focused pane, and `esc` followed by a key that is not bound sends both keys to the pane, so editors keep working. Set `keys.prefix = "ctrl+b"` in `config.toml` for the tmux-style prefix.

## Command palette

Press `prefix+?` at any time to open the command palette. It lists every command with its current shortcut, grouped by area.
...
- The `prefix mode` row is special: whatever you press becomes the prefix key itself.
