...
## Scrolling the pane scrollback

`PageUp` and `PageDown` scroll the focused pane's scrollback by one screen. Unlike most
Herdr shortcuts these are direct bindings, so they need no prefix. They yield to the pane
when it owns page keys — inside `less`, `vim`, or any full-screen app, the keys page that
app's content instead.

Both are rebindable as `keys.scroll_page_up` and `keys.scroll_page_down`, and there are two
further actions, `keys.scroll_half_page_up` and `keys.scroll_half_page_down`, that are unset
by default. A binding that carries a modifier always scrolls the scrollback and never yields
to the pane.
...
[ui]
page_scroll_lines = 20   # unset means one pane viewport
```
