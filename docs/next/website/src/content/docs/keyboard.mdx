...
The prefix solves this. Press the prefix key, default `ctrl+s`, and Herdr enters navigation mode: keys stop going to your terminal and run Herdr commands instead. `prefix+t` means: press `ctrl+s`, release, then press `t`. One reserved key instead of dozens.

Navigation mode stays open until you leave it, so you can run several commands from one press. A hint line at the bottom of the screen lists what is bound. `space` opens the command palette, `↑`/`↓` switch workspaces, `←`/`→` switch tabs, and `n`/`e`/`i`/`a` move between panes. All of those keep navigation mode open, so you can walk the whole session from one prefix press.

There are three ways out. `enter` commits the walk and leaves navigation mode on whatever workspace, tab, and pane you landed on. `esc` cancels it and returns focus to the pane that was focused before you opened navigation mode. Pressing the prefix key again leaves and sends a literal prefix to the focused pane, so with the default prefix `ctrl+s ctrl+s` keeps programs that use `ctrl+s` usable. A key that is not bound does nothing and keeps you in navigation mode. Set `keys.prefix = "ctrl+b"` in `config.toml` for the tmux-style prefix; then `ctrl+b ctrl+b` sends a literal `ctrl+b`.
...
| Move between panes | `prefix+n/e/i/a` |
...
```toml
[keys]
focus_pane_left = ["prefix+n", "ctrl+alt+h"]
focus_pane_down = ["prefix+e", "ctrl+alt+j"]
focus_pane_up = ["prefix+i", "ctrl+alt+k"]
focus_pane_right = ["prefix+a", "ctrl+alt+l"]
previous_tab = ["ctrl+alt+["]
next_tab = ["ctrl+alt+]"]
new_tab = ["prefix+t", "ctrl+alt+c"]
