...
Keyboard control is optional; the mouse covers everything. Press `esc` to enter navigation mode, then press action keys until you press `esc` again. `esc space` opens the command palette, which lists every command, runs the selected one, and rebinds shortcuts inline.
