...
Keyboard control is optional; the mouse covers everything. Press `esc` to enter prefix mode, then press an action key. `esc ?` opens the command palette, which lists every command, runs the selected one, and rebinds shortcuts inline.
