...
- `prefix+shift+k` now invokes the installed Sesh plugin workspace picker; install it through Herdr's managed plugin path with `herdr plugin install fullerzz/herdr-plugin-sesh`. Pane swap-up moved to `prefix+ctrl+k`.
